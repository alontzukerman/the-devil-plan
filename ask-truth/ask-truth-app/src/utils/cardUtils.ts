...
export interface Card {
    id: string; // Unique identifier, e.g., "KH" or "H-13"
    suit: Suit;
    rank: Rank;
}
...
export const getSuitDisplaySymbol = (suit: Suit): string => {
    ...
    if (suit === 'C') return '♣';
    if (suit === 'S') return '♠';
    return suit; // Fallback, should not happen with typed Suit
};
...
export const sortSelectedSeries = (cards: Card[]): Card[] => {
    ...
    // We can define a preferred suit order for display consistency if needed, or just iterate Object.keys
    const suitOrder: Suit[] = ['H', 'D', 'C', 'S']; // Example consistent order
...
/**
 * Validates if a selected series is valid according to game rules (intra-suit sorting).
 * This is mostly ensured by sortSelectedSeries if the input to that is the *actual* selected series.
 * However, this function can explicitly check if a pre-sorted list is valid.
 */
